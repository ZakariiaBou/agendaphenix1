...
<?php
  ...
  $lienAvant = mktime(12,0,0,$moisEnCours-1,$jourEnCours,$anneeEnCours);
  $lienApres = mktime(12,0,0,$moisEnCours+1,$jourEnCours,$anneeEnCours);
  ...
  if (!empty($sChoix)) {
    ...
    function afficheCase($leJour, $nbJour, $sUtilID,$aUtilDecalageHoraire,$aUtilDoubleJours, $AffTZ, $aHeureDebutJourneeUtil ,$aHeureFinJourneeUtil) {
      ...
      global $decalageHoraire, $tzGmt, $tzEte, $tzHiver, $aUtiltzGmt, $aUtiltzEte, $aUtiltzHiver, $formatHeure, $aUtiltzGmt, $aUtiltzEte, $aUtiltzHiver,$localTime, $CalJourFerie, $CalJourSelection , $CalJourEvenement;
      ...
      $UtilDecalageHoraire = calculDecalageH($aUtiltzGmt[$sUtilID],$aUtiltzEte[$sUtilID],$aUtiltzHiver[$sUtilID],$tsJour);
      $aDecalageHoraire = calculDecalageH($tzGmt,$tzEte,$tzHiver,$tsJour);
      ...
      $aJournee[1][6] = $aUtilDecalageHoraire - $UtilDecalageHoraire + $aDecalageHoraire - $decalageHoraire;
      if ($AffTZ=="O") {
        $aJournee[1][7] = $UtilDecalageHoraire - $aDecalageHoraire;
      } else {
        $aJournee[1][7] = 0;
      }
      ...
      while ($enr=$DB_CX->DbNextRow()) {
        ...
        if ($AffTZ=="O") {
          ...
          list($Stk_debr,$Stk_finr,$enr['dateCreation'],$enr['dateModif']) = decaleNote($aUtiltzGmt[$sUtilID],$aUtiltzEte[$sUtilID],$aUtiltzHiver[$sUtilID],$leJour,$enr['dateNote'],$enr['age_heure_debut'],$enr['age_heure_fin'],$enr['age_date_creation'],$enr['age_date_modif']);
        }
      ...
      if (($sUtilID != $idUser) and $aJournee[1][6] < 0) {
        $decalHP1 = $aJournee[1][6];
      } else {
        $decalHP1 = 0;
      }
      if (($sUtilID != $idUser) and $aJournee[1][6] > 0) {
        $decalHP2 = $aJournee[1][6];
      } else {
        $decalHP2 = 0;
      }
      if (!$aUtilDoubleJours) {
        for ($i=($iHeureMin+$decalHP1-1); $i<floor($aHeureDebutJourneeUtil+$decalHP1); $i++)
          if ($aJournee[floor($i-$iHeureMin)][0]=="0")
            $aJournee[floor($i-$iHeureMin)][0] = "2";
        for ($i=floor($aHeureFinJourneeUtil+$decalHP1+0.75); $i<=ceil($iHeureMax+$decalHP2); $i++)
          if ($aJournee[floor($i-$iHeureMin)][0]=="0")
            $aJournee[floor($i-$iHeureMin)][0] = "2";
      } else {
        for ($i=floor($aHeureDebutJourneeUtil+$decalHP1); $i<=ceil($aHeureFinJourneeUtil+$decalHP1); $i++)
          if ($aJournee[floor($i-$iHeureMin)][0]=="0")
            $aJournee[floor($i-$iHeureMin)][0] = "2";
      }
      $Class="";
      for ($i=0;$i<$iDureeJournee;$i++) {
        ...
        if ($aJournee[$i][0]!="2" && $aJournee[$i][0]!="0") {
          ...
          if ($droit_NOTES >= _DROIT_NOTE_STANDARD_SANS_APPR && (($sUtilID==$idUser) || (($aAutoAffect[$sUtilID]==1) || ($droit_NOTES >= _DROIT_NOTE_MODIF_CREATION and $droit_AGENDAS >= _DROIT_AGENDA_TOUS)))) {
            ...
            $T_Note=(floor($i+$iHeureMin+$aJournee[1][7]) < ($aHeureDebutJourneeUtil+$aJournee[1][7]) ? ($aHeureDebutJourneeUtil+$aJournee[1][7]) : floor($i+$iHeureMin+$aJournee[1][7]));
            $sOutput .= "    <DIV style=\"".$Class.$Class1."cursor:".$cursor."; height:".$hdiv."px; font-size:8px; font-weight:normal; background-color:".$BordCouleur.";\" onclick=\"javascript: nvNoteG('".$tsJour."','".$T_Note."','','".$sUtilID."','".(($AffTZ=="O") ? $sUtilID : "")."');\" title=\"".sprintf(trad("PLGL_CREER_NOTE"), afficheHeure($T_Note,$T_Note))."\">&nbsp;</DIV>\n";
          } else {
            $lienNote = "";
            $cursor = "default";
    ...
    $iHeureMax = "0";
    //Preparation au decalage horaire
    $decalageHoraire = calculDecalageH($tzGmt,$tzEte,$tzHiver,mktime(12,0,0,$moisEnCours,1,$anneeEnCours));
    $bDecalageHoraire = calculDecalageH($tzGmt,$tzEte,$tzHiver,mktime(12,0,0,$moisEnCours+1,0,$anneeEnCours));
    ...
    while ($enr = $DB_CX->DbNextRow()) {
      ...
      $aUtilDecalageHoraire[$enr['util_id']] = calculDecalageH($aUtiltzGmt[$enr['util_id']],$aUtiltzEte[$enr['util_id']],$aUtiltzHiver[$enr['util_id']],mktime(12,0,0,$moisEnCours,1,$anneeEnCours));
    ...
    // Info sur les utilisateurs selectionnes
    $DB_CX->DbQuery("SELECT util_id, CONCAT(".$FORMAT_NOM_UTIL.") AS nomUtil, util_autorise_affect FROM ${PREFIX_TABLE}utilisateur WHERE util_id IN (".$sChoix.") ORDER BY nomUtil");
    // Tableau contenant les id => nom des utilisateurs
    $aUtilPartage = array();
    while ($enr=$DB_CX->DbNextRow()) {
      $aUtilPartage[$enr['util_id']] = $enr['nomUtil'];
    ...
    while (list($sId,$sNom)=each($aUtilPartage)) {
      if (($NblignUtil % 10)==0) {
       ...
       //Entete du tableau
        for ($i=1; $i<=$nbJourMois; $i++) {
          ...
          $tsJour = mktime(12,0,0,$moisEnCours, $i, $anneeEnCours);
      ...
      if ($AffTZ=="O")
        $styleUTC = "<BR><SPAN style=\"font-weight:normal;font-style:normal;\"><SMALL>".sprintf(trad("PLGL_FUSEAU"), (($aUtiltzGmt[$sId]<0) ? "-" : "+").afficheHeure(floor(abs($aUtiltzGmt[$sId])),abs($aUtiltzGmt[$sId])), $aUtiltzLibelle[$sId])."</SMALL></SPAN>";
      else
        $styleUTC = "";
      ...
      if ($autoriseConsult || ($ckAffCache=="O")) {
        ...
        for ($i=1; $i<=$nbJourMois; $i++) {
          ...
          if (substr($vSemaineType[$sId],date("w",mktime(12,0,0,$moisEnCours, $i, $anneeEnCours)),1)=="0") {
            $aHeureDebutJourneeUtilTmp = $aHeureFinJourneeUtil[$sId];
          } else {
            $aHeureDebutJourneeUtilTmp = $aHeureDebutJourneeUtil[$sId];
          }
          afficheCase($leJour, $i, $sId,$aUtilDecalageHoraire[$sId],$aUtilDoubleJours[$sId],$AffTZ,$aHeureDebutJourneeUtilTmp,$aHeureFinJourneeUtil[$sId]);
        }
        $sOutput="";
