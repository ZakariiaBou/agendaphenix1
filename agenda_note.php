...
<?php
  $DB_CX->DbQuery("SELECT util_debut_journee, util_duree_note, util_rappel_delai, util_rappel_type, util_rappel_email, util_fin_journee FROM ${PREFIX_TABLE}utilisateur WHERE util_id=".$idUser);
  ...
  $DB_CX->DbQuery("SELECT util_id, tzn_gmt, tzn_date_ete, tzn_heure_ete, tzn_date_hiver, tzn_heure_hiver FROM ${PREFIX_TABLE}utilisateur, ${PREFIX_TABLE}timezone WHERE (util_id =".$decalH.") AND (tzn_zone=util_timezone)");
  while ($enr=$DB_CX->DbNextRow()) {
    $UtiltzGmt=$enr['tzn_gmt'];
    $UtiltzEte = calculBasculeDST($enr['tzn_date_ete'],gmdate("Y"),$enr['tzn_heure_ete'],$UtiltzGmt,0);
    $UtiltzHiver = calculBasculeDST($enr['tzn_date_hiver'],gmdate("Y"),$enr['tzn_heure_hiver'],$UtiltzGmt,1);
  }
  if (!isset($decalH)) {
    $decalageHoraire = calculDecalageH($tzGmt,$tzEte,$tzHiver,mktime(12,0,0,$moisEnCours,$jourEnCours,$anneeEnCours));
  } else {
    $decalageHoraire = calculDecalageH($UtiltzGmt,$UtiltzEte,$UtiltzHiver,mktime(12,0,0,$moisEnCours,$jourEnCours,$anneeEnCours));
  }

  $enr['ageDate'] = date("d/m/Y",$sd);
  $enr['age_heure_debut'] = (isset($hD) && !empty($hD)) ? $hD+(($id) ? $decalageHoraire : 0) : $debutJournee;
  ...
  if ($id) {
    ...
    if (!isset($edit) && !isset($idMere)) {
      ...
      // On recherche si on est en train de modifier une occurence d'une note recurrente
      ...
      if ($DB_CX->DbResult(0,0)) {
        ...
        // pour offrir la possibilite de modifier soit cette occurence, soit toute la serie
    ...
    if (!$idAgeMere) {
      ...
      $DB_CX->DbQuery("SELECT age_id, DATE_FORMAT(age_date,'%d/%m/%Y') AS ageDate, age_heure_debut, age_heure_fin, age_ape_id, age_periode1, age_periode2, age_periode3, age_periode4, age_plage, age_plage_duree, age_libelle, age_detail, age_rappel, age_rappel_coeff, age_email, age_prive, age_couleur, age_aty_id, age_disponibilite, age_lieu, age_cal_id, age_email_contact, age_util_id FROM ${PREFIX_TABLE}agenda WHERE age_id=".$id.$testDroit." AND age_aty_id!=1");
...
<?php
    ...
    // Affichage abrege en cas de modification d'une occurence d'une note recurrente
...
<?php
    // Fin de l'affichage abrege en cas de modification d'une occurence d'une note recurrente
    }
?>
...
<?php
  ...
  // ou uniquement l'occurence choisie (dans ce cas le formulaire ne contiendra pas les informations de periodicite)
  if ($idAgeMere) {
...
?>
  ...
  <FORM action="agenda_traitement.php" method="post" name="Form1">
    ...
    <INPUT type="hidden" name="tzGmt" value="<?php if (!isset($decalH)) echo $tzGmt; else echo $UtiltzGmt; ?>">
    <INPUT type="hidden" name="tzEte" value="<?php if (!isset($decalH)) echo $tzEte; else echo $UtiltzEte; ?>">
    <INPUT type="hidden" name="tzHiver" value="<?php if (!isset($decalH)) echo $tzHiver; else echo $UtiltzHiver; ?>">
...
?>
  ...
  <TR bgcolor="<?php echo $bgColor[++$iColor%2]; ?>" height="21">
    ...
    <TD class="tabInput" nowrap><INPUT type="text" class="Texte" name="ztLieu" value="<?php echo htmlspecialchars($enr['age_lieu']); ?>" style="width:469px" maxlength="230"></TD>
  </TR>
  <TR bgcolor="<?php echo $bgColor[++$iColor%2]; ?>">
  ...
  <TR bgcolor="<?php echo $bgColor[++$iColor%2]; ?>" height="21">
    <TD class="tabIntitule"><?php echo trad("NOTE_LIB_HORAIRES");?></TD>
...
?>
  <TR bgcolor="<?php echo $bgColor[++$iColor%2]; ?>">
    ...
    <TD class="tabInput"><TABLE cellspacing="0" cellpadding="0" width="100%" border="0">
      ...
      <TR>
        <TD><SELECT name="zlUtilisateur" id="zlUtilisateur" size="8" multiple style="width:210px; border:<?php echo $FormulaireBordureInput; ?>;">
<?php
...
?>
        ...
        <TD><SELECT name="zlParticipant" id="zlParticipant" size="8" multiple style="width:210px; border:<?php echo $FormulaireBordureInput; ?>;"></SELECT></TD>
      </TR>
...
<?php
      ...
      $rdP1 = " checked"; $rdP2 = ""; $ztP = "10"; $ztDateFin = date("d/m/Y",mktime(12,0,0,$m+1,$d,date("Y",$sd)));
...
?>
          ...
          <LABEL for="mois2"><INPUT type="radio" name="rdM" id="mois2" value="2" class="Case"<?php echo $rdM2; ?>> <?php echo trad("NOTE_LE");?></LABEL> <SELECT name="zlM2" onFocus="document.Form1.rdM[1].checked='true';">
...
?>
  ...
  <BR><CENTER>
    ...
    <?php if (($Autoriser_Modif_Affect) and (($ztAction == "UPDATE") or ($edit != "occ"))) { ?><INPUT type="button" name="btDuplique" value="<?php echo trad("NOTE_BT_DUPLIQUER");?>" onclick="javascript: dupNote(document.Form1);" class="Bouton"><?php } ?>
    </CENTER>
  </FORM>
