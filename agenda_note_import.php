<?php
  ...
  *  option) any later version.                                              *
  ...
  if (!empty($id)) {
    ...
    $_POST['ztDate'] = date("d")."/".date("m")."/".(date("Y")-5);
    $_POST['ztDate2'] = date("d")."/".date("m")."/".(date("Y")+5);
      ...
      if (strtolower($fic_ext[1])!="ics") {
        ...
        for ($ligne_fic=0;$ligne_fic<=count($fcontents);$ligne_fic++) {
          ...
          if (trim(($tabCtt[0])=="BEGIN") && (trim($tabCtt[1])=="VEVENT")) {
            ...
            $zlCouleur=$ztDate=$ckTypeNote=$ztParticipant=$ztLibelle=$ztLieu=$ztDetail=$rdPrive=$zlHeureDebut=$zlHeureFin=$vSemaineType=$rdRappel=$zlR1=$zlR2=$ckEmail=$rdDispo=$zlContactAssocie=$idAge="";
          }
          if ($nbNote>0) {
            // on traite la note
          ...
          if (trim(($tabCtt[0])=="END") && (trim($tabCtt[1])=="VEVENT")) {
            // Date du jour pour gerer les evenements faits ou non
            // (par defaut date anterieure a aujourd'hui aco_termine => 1)
            $j = gmdate("d");
            $j_nom = strtoupper(substr(gmdate("D"),0,2));
            $m = gmdate("m");
            $an = gmdate("Y");
            $date_aujourdhui = $an."-".$m."-".$j;
            ...
            list($jour_dem_deb,$mois_dem_deb,$an_dem_deb) = explode("/",$_POST['ztDate']);
            $date_dem_deb = $an_dem_deb."-".$mois_dem_deb."-".$jour_dem_deb;
            list($jour_dem_fin,$mois_dem_fin,$an_dem_fin) = explode("/",$_POST['ztDate2']);
            ...
            if ($age_date_create!="") {
              ...
              $age_date_create = $date_create."-".$mois_create."-".$jour_create." ".$heure_create.":".$min_create.":00";
            } else {
              $age_date_create = gmdate("Y-m-d")." ".gmdate('H').":".gmdate('i').":00";
            }
            ...
            if (($age_date_modif!="") || ($age_date_modif!="00000000T000000Z")) {
              ...
              $age_date_modif = $date_modif."-".$mois_modif."-".$jour_modif." ".$heure_modif.":".$min_modif.":00";
            } else {
            ...
            if ($note_TZ != "") {
            // Conversion des dates recues en UTC
              $DB_CX->DbQuery("SELECT tzn_libelle, tzn_gmt, tzn_date_ete, tzn_heure_ete, tzn_date_hiver, tzn_heure_hiver FROM ${PREFIX_TABLE}timezone WHERE tzn_zone='".$note_TZ."'");
              if ($DB_CX->DbNumRows()) {
                $tzLibelle_N = htmlentities($DB_CX->DbResult(0,0));
                $tzGmt_N = $DB_CX->DbResult(0,1);
                $tzEte_N = calculBasculeDST($DB_CX->DbResult(0,2),gmdate("Y"),$DB_CX->DbResult(0,3),$tzGmt_N,0);
                $tzHiver_N = calculBasculeDST($DB_CX->DbResult(0,4),gmdate("Y"),$DB_CX->DbResult(0,5),$tzGmt_N,1);
                // Conversion des dates demandes en Local
                $decalageHoraireD = calculDecalageH($tzGmt_N,$tzEte_N,$tzHiver_N,mktime($heure_deb_h,$heure_deb_m,0,$mois_debut,$jour_debut,$an_debut));
                $decalageHoraireF = calculDecalageH($tzGmt_N,$tzEte_N,$tzHiver_N,mktime($heure_fin_h,$heure_fin_m,0,$mois_debut,$jour_debut,$an_debut));
                $heure_deb -= $decalageHoraireD;
                $heure_fin -= $decalageHoraireF;
                // Normalisation de la date et de l'heure de debut
                if ($heure_deb < 0) {
                  $heure_deb += 24;
                  $date_debut = date("Y-m-d",mktime(12,0,0,$mois_debut,$jour_debut-1,$an_debut));
                }
                if ($heure_deb >= 24) {
                  $heure_deb -= 24;
                  $date_debut = date("Y-m-d",mktime(12,0,0,$mois_debut,$jour_debut+1,$an_debut));
                ...
                if ($heure_fin > 24) $heure_fin -= 24;
              }
            }
            // Conversion des dates demandes en UTC
            $decalageHoraire = calculDecalageH($tzGmt,$tzEte,$tzHiver,mktime(12,0,0,$mois_dem_deb,$jour_dem_deb,$an_dem_deb));
            $date_dem_deb = date("Y-m-d H.i",mktime(0-floor($decalageHoraire),0-($decalageHoraire*60)%60,0,$mois_dem_deb,$jour_dem_deb,$an_dem_deb));
            $decalageHoraire = calculDecalageH($tzGmt,$tzEte,$tzHiver,mktime(12,0,0,$mois_dem_fin,$jour_dem_fin,$an_dem_fin));
            $date_dem_fin = date("Y-m-d H.i",mktime(24-floor($decalageHoraire),0-($decalageHoraire*60)%60,0,$mois_dem_fin,$jour_dem_fin,$an_dem_fin));
            ...
            if ((($date_debut." ".$heure_deb)>=$date_dem_deb) && (($date_debut." ".$heure_deb)<=$date_dem_fin)) {
                ...
                if ($note_rrule=="") {
                  ...
                  if (!empty($vFreq)) {
                    ...
                    if (!empty($vUntil)) {
                      ...
                      // Occurrences
                      $rdPlage = 1;
                      $ztP = ($vCount>0) ? $vCount : 1; // Si pas d'occurence on transforme en 1 fois
                    }
              ...
              if ($note_TZ != "") {
                $decalageHoraire_N = calculDecalageH($tzGmt_N,$tzEte_N,$tzHiver_N,mktime(12,0,0,$mois_debut,$jour_debut,$an_debut));
                $decalageHoraire = calculDecalageH($tzGmt,$tzEte,$tzHiver,mktime($heure_deb_h,$heure_deb_m,0,$mois_debut,$jour_debut,$an_debut))-$decalageHoraire_N;
              } else {
                $decalageHoraire = calculDecalageH($tzGmt,$tzEte,$tzHiver,mktime($heure_deb_h,$heure_deb_m,0,$mois_debut,$jour_debut,$an_debut));
              }
              $heure_deb_loc = ($heure_deb_h.".".substr($heure_deb_tmp,2,2)) + $decalageHoraire;
              $heure_fin_loc = ($heure_fin_h.".".substr($heure_fin_tmp,2,2)) + $decalageHoraire;
              $date_deb_loc = date("d/m/Y",mktime(12,0,0,$mois_debut,$jour_debut,$an_debut));
              // on normalise la date et l'heure de debut
              if ($heure_deb_loc < 0) {
                $heure_deb_loc += 24;
                $date_deb_loc = date("d/m/Y",mktime(12,0,0,$mois_debut,$jour_debut-1,$an_debut));
              }
              if ($heure_deb_loc >= 24) {
                $heure_deb_loc -= 24;
                $date_deb_loc = date("d/m/Y",mktime(12,0,0,$mois_debut,$jour_debut+1,$an_debut));
              ...
              if ($heure_fin_loc <= 0) $heure_fin_loc += 24;
              if ($heure_fin_loc > 24) $heure_fin_loc -= 24;
                ...
                if ($note_rrule=="") {
                  ...
                  if (($vFreq=="D") || ($vFreq=="W") || ($vFreq=="M") || ($vFreq=="Y")) {
                    ...
                    if (substr($derniereOption,0,1)=="#") {
                      ...
                      // La derniere option n'est pas une date ou un nombre d'occurence  => C'est un UNTIL infini, PHENIX ne le gerant pas on fixe ARBITRAIREMENT la date de fin au 31/12/annee courante + 10 ans
                      $vUntil = (gmdate("Y")+10)."1231";
  ...
  function importCSV($fileName,&$err) {
    global $DB_CX, $PREFIX_TABLE, $idUser;
    global $zlPeriodicite,$zlCouleur,$rdA,$zlA1,$zlA2,$ztP,$zlP1,$zlP2,$zlP3,$rdQ,$ztQ,$zlH,$ztH,$rdM,$zlM1,$zlM2,$zlM3,$idUser,$ztDate,$ckTypeNote,$idAge,$ztParticipant,$ztLibelle,$ztDetail,$rdPrive,$zlHeureDebut,$zlHeureFin,$age_date_create,$age_date_modif;
    global $tzGmt,$tzEte,$tzHiver;
    $age_date_create="";
    ...
    if (file_exists($fileName)) {
      ...
      if (strtolower($fic_ext[1])!="csv") {
        ...
        // Date du jour (au format phenix pour comparer les date des notes : terminee ou non)
        $j = gmdate("d");
        $j_nom = strtoupper(substr(gmdate("D"),0,2));
        $m = gmdate("m");
        $an = gmdate("Y");
        ...
        for ($i=1;$i<=count($fcontents);$i++) {
          ...
          if (($ajout_en_cours==0) && ($ajout_ok==0)) {
            ...
            $note_description = "";
            $note_location = "";
            $zlPeriodicite=$zlCouleur=$rdA=$zlA1=$zlA2=$ztP=$zlP1=$zlP2=$zlP3=$rdQ=$ztQ=$zlH=$ztH=$rdM=$zlM1=$zlM2=$zlM3=$ztDate=$ckTypeNote=$idAge=$ztParticipant=$ztLibelle=$ztDetail=$rdPrive=$zlHeureDebut=$zlHeureFin="";
          }
          ...
          if ($ajout_ok==1) {
            ...
            if (($date_debut>=$date_dem_deb) && ($date_debut<=$date_dem_fin)) {
              $id_age="";
              //Decalage des notes au format UTC
              $decalageHoraire = calculDecalageH($tzGmt,$tzEte,$tzHiver,mktime($heure_deb_h,$heure_deb_m,0,$mois_debut,$jour_debut,$an_debut));
              $date_deb_utc = mktime(12,0,0,$mois_debut,$jour_debut,$an_debut);
              $heure_deb_utc = ($heure_deb_h.".".$heure_deb_m) - $decalageHoraire;
              $heure_fin_utc = ($heure_fin_h.".".$heure_fin_m) - $decalageHoraire;
              // on normalise la date et l'heure de debut
              if ($heure_deb_utc < 0) {
                $heure_deb_utc += 24;
                $date_deb_utc = mktime(12,0,0,$mois_debut,$jour_debut-1,$an_debut);
              }
              if ($heure_deb_utc >= 24) {
                $heure_deb_utc -= 24;
                $date_deb_utc = mktime(12,0,0,$mois_debut,$jour_debut+1,$an_debut);
              ...
              // on normalise l'heure de fin
              if ($heure_fin_utc <= 0) $heure_fin_utc += 24;
              if ($heure_fin_utc > 24) $heure_fin_utc -= 24;
              ...
              if ($DB_CX->DbNumRows()) {
                ...
                $age_date_modif = gmdate("Y-m-d")." ".gmdate('H').":".gmdate('i').":00";
                // Generation du rapport et Enregistrement de la note si import valide
        ...
        <TR bgcolor="<?php echo $CalepinFondMessage; ?>">
          <TD align="left" colspan="2" class="bordTLRB" style="padding-left:3px;padding-right:3px;">
            ...
            <TR>
              ...
              <TD><INPUT type="text" class="Texte" name="ztDate" id="ztDate" size=12 maxlength=10 value="<?php echo $date_deb; ?>" title="<?php echo trad("NOTEIMP_FORMAT_DATE");?>" onKeyPress="return onlyChar(event);">&nbsp;<INPUT type="button" id="btCal" value="..." class="Picklist" style="height:16px" title="<?php echo trad("NOTEIMP_AFFICHE_CALENDRIER");?>"></TD>
              <TD rowspan="2" align="center"><?php echo trad("NOTEIMP_INCHANGER_DATES");?></TD>
            </TR><TR>
              <TD><B><?php echo trad("NOTEIMP_LIB_DATE_FIN");?> :</B></TD>
              <TD><INPUT type="text" class="Texte" name="ztDate2" id="ztDate2" size=12 maxlength=10 value="<?php echo $date_fin; ?>" title="<?php echo trad("NOTEIMP_FORMAT_DATE");?>" onKeyPress="return onlyChar(event);">&nbsp;<INPUT type="button" id="btCal2" value="..." class="Picklist" style="height:16px" title="<?php echo trad("NOTEIMP_AFFICHE_CALENDRIER");?>"></TD>
            </TR>
            </TABLE><BR>
          </TD>
        </TR>
...
?>
  ...
  <!--
    ...
    Calendar.setup( {
      inputField : "ztDate2",    // ID of the input field
      ifFormat   : "%d/%m/%Y",  // the date format
...
<?php
  ...
  // Fonction d'ajustement des notes si heures d'hivers
  function maj_tz($HeureDebut,$HeureFin,$Date) {
    global $tzEte, $tzHiver;
    $tabDate = explode("-",$Date);
    $decalHD = calculDecalageH(0,$tzHiver,$tzEte,mktime(floor($HeureDebut),($HeureDebut*60)%60,0,$tabDate[1],$tabDate[2],$tabDate[0]));
    $HeureDebut += $decalHD;
    $decalHF = calculDecalageH(0,$tzHiver,$tzEte,mktime(floor($HeureFin),($HeureFin*60)%60,0,$tabDate[1],$tabDate[2],$tabDate[0]));
    $HeureFin += $decalHF;
    if ($HeureFin < 0) $HeureFin += 24;
    if ($HeureFin < 0) $HeureFin += 24;
    if ($HeureFin >= 24) $HeureFin -= 24;
    if ($HeureFin >= 24) $HeureFin -= 24;
    return array($HeureDebut,$HeureFin);
  }
  // Fonction d'insertion / modification des notes
  function import_cal(){
    global $DB_CX, $PREFIX_TABLE, $idUser, $tzGmt, $tzEte, $tzHiver;
    ...
    if ($idAge) {
      ...
      $sql .= "VALUES (0,".$idUser.",".$ckTypeNote.",'".$ztDate."',".$zlHeureDebut.",".$zlHeureFin.",".$zlPeriodicite.",".$periode1.",".$periode2.",".$periode3.",".$periode4.",".$rdPlage.",".($nbOccurence + $dateMax).",'".$ztLibelle."','".$ztDetail."',".$zlR1.",".$zlR2.",".$ckEmail.",".$rdPrive.",'".$zlCouleur."',".count($idParticipant).",".$idUser.",".$rdDispo.",'".$age_date_create."','".$age_date_modif."',".$idUser.",'".$ztLieu."',".$zlContactAssocie.")";
      $DB_CX->DbQuery($sql);
    ...
    if ($idAge) {
      // Requete generique
      $sql = "INSERT INTO ${PREFIX_TABLE}agenda (age_mere_id,age_util_id,age_aty_id,age_date,age_heure_debut,age_heure_fin,age_ape_id, age_periode1, age_periode2, age_periode3, age_periode4, age_plage, age_plage_duree, age_libelle, age_detail, age_rappel, age_rappel_coeff, age_email, age_prive, age_couleur, age_nb_participant, age_createur_id, age_disponibilite, age_date_creation, age_date_modif, age_modificateur_id, age_lieu, age_cal_id) ";
      $sql .= "VALUES (".$idAge.",".$idUser.",".$ckTypeNote.",";
      $sql1= ",".$zlPeriodicite.",".$periode1.",".$periode2.",".$periode3.",".$periode4.",".$rdPlage.",".($nbOccurence + $dateMax).",'".$ztLibelle."','".$ztDetail."', ".$zlR1.",".$zlR2.",".$ckEmail.",".$rdPrive.",'".$zlCouleur."',".count($idParticipant).",".$idUser.",".$rdDispo.",'".$age_date_create."','".$age_date_modif."',".$idUser.",'".$ztLieu."',".$zlContactAssocie.")";
      if ($rdPlage == 1) {
          ...
          case 2 : // Quotidienne
            if ($rdQ == 1) {
              for ($i=1;$i<$nbOccurence;$i++) {
                ...
                $endNote = ($tsNote > $tsNow) ? 0 : 1;
                $alert = ($tsNote > $tsAlert && $zlR1) ? 0 : 1;
                list($HeureDebut,$HeureFin)=maj_tz($zlHeureDebut,$zlHeureFin,date("Y-m-d",$tsNote));
                $DB_CX->DbQuery($sql."'".date("Y-m-d",$tsNote)."',".$HeureDebut.",".$HeureFin.$sql1);
                // Enregistrement des personnes concernees
