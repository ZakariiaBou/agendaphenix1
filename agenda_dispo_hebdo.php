...
<?php
  ...
  if (!empty($sChoix)) {
    ...
    $DB_CX->DbQuery("SELECT util_id, util_debut_journee, util_fin_journee, util_semaine_type FROM ${PREFIX_TABLE}utilisateur WHERE util_id IN (".$sChoix.") ORDER BY util_id");
...
<?php
    ...
    for ($j=0;$j<7;$j++) {
      ...
      $sOutput .= "    <TD class=\"nomUtil\"><A href=\"javascript: affJour('".$leJour."');\" title=\"".trad("DISPOH_DETAIL_UTIL")."\"><B>&nbsp;".$tabJour[date("w",$leJour)]."&nbsp;<BR>".date("d/m",mktime(12,0,0,$moisEnCours,$premierJourSemaine+$j,$anneeEnCours))."</B></A></TD>\n";
      ...
      $sql  = "SELECT age_heure_debut, age_heure_fin, age_aty_id, aco_util_id, $age_date AS dateNote, age_date_creation, age_date_modif";
