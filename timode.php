  ...
  function calculBasculeDST($bascule,$annee,$heure,$gmt,$dst) {
    if (!empty($bascule)) {
      ...
      if (preg_match('/(\d+):(\d+)(\w?)/',$heure,$res)) {
        ...
        $heures = $res[1];
        $minutes = $res[2];
        if ($res[3]=='u') $heures += $gmt + $dst;
      }
      $date = gmmktime($heures,$minutes,0,$tabMois[$mois],$jour,$annee);
    }
    return $date;
  }
// ----------------------------------------------------------------------------
...
// ----------------------------------------------------------------------------
  function decaleNote($gmt,$ete,$hiver,$dateJour,$dateNote,$hdeb,$hfin,$dateCrt) {
    $tabDate = explode("-",$dateNote);
    $decalHD = calculDecalageH($gmt,$ete,$hiver,mktime(floor($hdeb),($hdeb*60)%60,0,$tabDate[1],$tabDate[2],$tabDate[0]));
    $decalHF = calculDecalageH($gmt,$ete,$hiver,mktime(floor($hfin),($hfin*60)%60,0,$tabDate[1],$tabDate[2],$tabDate[0]));
    $hdeb += $decalHD;
    $hfin += $decalHF;
    if ($hdeb < 0) $hdeb += 24;
    if ($hfin < 0) $hfin += 24;
    ...
    if ($ageDateCrt[1]!="00:00:00") {
      ...
      $decalHC = calculDecalageH($gmt,$ete,$hiver,mktime($hrCrt[0],$hrCrt[1],$hrCrt[2],$dtCrt[1],$dtCrt[2],$dtCrt[0]));
      $dateCrt = date("Y-m-d H:i:s",mktime($hrCrt[0]+floor($decalHC),$hrCrt[1]+($decalHC*60)%60,$hrCrt[2],$dtCrt[1],$dtCrt[2],$dtCrt[0]));
    }
    $dateCrt = strftime(trad("COMMUN_FORMAT_DATE_SQL"),strtotime($dateCrt));
    return array($hdeb,$hfin,$dateCrt);
  }
// ----------------------------------------------------------------------------
...
// ----------------------------------------------------------------------------
  function prepareDecalageH($gmt,$ete,$hiver,$date) {
    ...
    $age_heure_debut = "(age_heure_debut+".($decalageHoraire+24).")%24";
    $age_heure_fin = "(age_heure_fin+".($decalageHoraire+24).")%24";
  }
// ----------------------------------------------------------------------------
...
// ----------------------------------------------------------------------------
  elseif ($v==2) {
    ...
    if ($id) {
      $DB_CX->DbQuery("SELECT age_id, age_libelle, age_detail, DATE_FORMAT(age_date,'%d/%m/%Y') AS ageDate, age_heure_debut, age_heure_fin, age_prive, age_rappel, age_rappel_coeff, age_aty_id FROM ${PREFIX_TABLE}agenda WHERE age_id=".$id." AND age_util_id=".$idUser." AND age_aty_id!=1");
...
// ----------------------------------------------------------------------------
  elseif ($v==3) {
    ...
    if ($ztAction == "INSERT") {
      ...
      $sql .= "VALUES (".$idUser.",".$ckTypeNote.",'".$ztDate."',".$zlHeureDebut.",".$zlHeureFin.",'".$ztLibelle."','".$ztDetail."',".$zlR1.",".$zlR2.",".$zlPartage.",".$idUser.", '".$dateCreation."',".$idUser.", '".$dateCreation."')";
      $DB_CX->DbQuery($sql);
...
// ----------------------------------------------------------------------------
  if ($v==1) {
    ...
    $DB_CX->DbQuery("SELECT age_id,age_aty_id,age_heure_debut,age_heure_fin,age_libelle,age_ape_id,age_util_id,age_detail,cal_id,CONCAT(cal_prenom,' ',cal_nom) AS nomContact,cal_util_id,cal_partage,$age_date AS dateNote,age_date_creation FROM ${PREFIX_TABLE}agenda LEFT JOIN ${PREFIX_TABLE}calepin ON cal_id=age_cal_id, ${PREFIX_TABLE}agenda_concerne WHERE age_id=aco_age_id AND aco_util_id=".$idUser." AND ($age_date='".date("Y-m-d",$sd)."' OR ($age_dateAvant='".date("Y-m-d",$sd)."' AND $age_heure_debut>=$age_heure_fin AND $age_heure_fin!=0 AND age_aty_id=2) OR (age_date LIKE '%".date("m-d",$sd)."' AND age_aty_id=1)) ORDER BY age_aty_id DESC, age_date, age_heure_debut ASC");
    $ligneAnniv = $ligneNote = "";
    for ($j=0;$j<$DB_CX->DbNumRows();$j++) {
      $enr = $DB_CX->DbNextRow();
      //Decalage des notes en fonction du fuseau horaire
      list($enr['age_heure_debut'],$enr['age_heure_fin'],$enr['dateCreation']) = decaleNote($tzGmt,$tzEte,$tzHiver,date("Y-m-d",$sd),$enr['dateNote'],$enr['age_heure_debut'],$enr['age_heure_fin'],$enr['age_date_creation']);
...
// ----------------------------------------------------------------------------
  elseif (!$v) {
    ...
    function afficheJour($nbJour, $leJour) {
      ...
      $numJour = ($tabOccupe[$nbJour]==1) ? "<a href=\"".$NOM_PAGE."?sid=".$sid."&v=1&sd=".mktime(12,0,0,intval($moisEnCours),$nbJour,$anneeEnCours)."\"><font color=\"#800000\">".$nbJour."</font></a>" : "<font color=\"#4924FF\">".$nbJour."</font>";
