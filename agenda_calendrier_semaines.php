<?php
  ...
  *  option) any later version.                                              *
  ...
  $indexJourAn = date("w",mktime(12,0,0,1,1,$crtYear));
  if ($indexJourAn == 0) $indexJourAn = 7;
  $premierLundi = mktime(12,0,0,1,2-$indexJourAn,$crtYear);
  // Premiere et Derniere semaine de l'annee
  $firstWeek  = date("W",mktime(12,0,0,1,1,$crtYear));
  $lastWeek   = date("W",mktime(12,0,0,12,31,$crtYear));
  $semaineMax = ($lastWeek==1) ? date("W",mktime(12,0,0,12,24,$crtYear)) : $lastWeek;
  $cpt = 1;
  $idx = 0;
  for($j=1;$cpt<$semaineMax;$j++) {
    ...
    for($i=1;$i<8;$i++) {
      if ($j==1 && $i==1 && $firstWeek!=1) {
        ...
        elseif ($debutSemaine == mktime(12,0,0,1,1,$crtYear))
        ...
        $idxSemaine = mktime(12,0,0,1,(2-$indexJourAn)+($idx*7),$crtYear);
