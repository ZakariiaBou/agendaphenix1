      ...
      if ((strtolower($fic_ext[1])!="dba") and (strtolower($fic_ext[1])!="dat")) {
        ...
        for ($nbNote=0; $nbNote<$NbEntrees; $nbNote++) {
          ...
          if ((dechex($note_statut & 128) != 80) && (dechex($note_statut & 4)!= 4)) {
            ...
            $date_debut = date("Y-m-d",mktime(0+floor($decalageHoraire),0+($decalageHoraire*60)%60,0,$mois_debut,$jour_debut,$an_debut));
            $an_debut = substr($date_debut,0,4);
            $mois_debut = substr($date_debut,5,2);
            $jour_debut = substr($date_debut,8,2);
            // Conversion des heures recues en Local
            $decalageHoraireD = calculDecalageH($tzGmt,$tzEte,$tzHiver,mktime($heure_deb_h,$heure_deb_m,0,$mois_debut,$jour_debut,$an_debut));
            $decalageHoraireF = calculDecalageH($tzGmt,$tzEte,$tzHiver,mktime($heure_fin_h,$heure_fin_m,0,$mois_fin,$jour_fin,$an_fin));
            $heure_deb -= $decalageHoraireD;
            $heure_fin -= $decalageHoraireF;
            // Normalisation de l'heure de debut
            if ($heure_deb < 0) {
              $heure_deb += 24;
            ...
            if ((($date_debut." ".$heure_deb)>=$date_dem_deb) && (($date_debut." ".$heure_deb)<=$date_dem_fin)) {
              ...
              $decalageHoraire = calculDecalageH($tzGmt,$tzEte,$tzHiver,mktime(12,0,0,$mois_debut,$jour_debut,$an_debut));
              $heure_deb_loc = $heure_deb + $decalageHoraire;
              $heure_fin_loc = $heure_fin + $decalageHoraire;
              ...
              addReport($importValide, floor($heure_deb_loc).trad("NOTEIMP_H").(substr($heure_deb_loc*100,-2)*0.6), floor($heure_fin_loc).trad("NOTEIMP_H").(substr($heure_fin_loc*100,-2)*0.6).$rep_info, $date_deb_loc, $note_summary);
            }
          }
        }
      }
