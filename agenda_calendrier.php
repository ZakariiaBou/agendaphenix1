...
<?php
  if ((($USER_SUBSTITUE==$idUser) and ($droit_NOTES >= _DROIT_NOTE_STANDARD_SANS_APPR)) or ($droit_NOTES >= _DROIT_NOTE_MODIF_CREATION)) {
    ...
    $DB_CX->DbQuery("SELECT DISTINCT mem_id, mem_titre, mem_contenu, mem_util_id FROM ${PREFIX_TABLE}memo WHERE (mem_util_id=".$idUser." OR (mem_util_id!=".$idUser." AND mem_partage='O')) ORDER BY mem_id ASC");
    ...
    <TD><TABLE cellspacing=\"0\" cellpadding=\"0\" width=\"100%\" border=\"0\" class=\"paddingDG3\">
      ...
      while ($enr=$DB_CX->DbNextRow()) {
        ...
        <TD width=\"100%\"><A".$lien." class=\"MemoFavorisTexte\" onmouseover=\"mtc('".addslashes(htmlspecialchars(nlTObr($enr['mem_titre'])))."','".addslashes(htmlspecialchars(nlTObr($enr['mem_contenu'])))."&nbsp;',330); return false;\" onmouseout=\"nd(); return true;\">".$enr['mem_titre']."</A></TD>
      </TR>\n");
      }
      echo ("    </TABLE></TD>
  </TR>\n");
