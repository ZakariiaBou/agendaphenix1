<?php
  ...
  *  option) any later version.                                              *
  ...
  if (!defined("_CONF_INC_LOADED") && file_exists("./install/install.php")) {
...
<?php message($msg); ?>
  ...
  <TABLE border="0" cellspacing="0" cellpadding="0" align="center"><TR><TD class="Copyright" align="center"><BR>Phenix <?php echo $APPLI_VERSION; ?> [phenix.gapi.fr]<BR>&copy; St&eacute;phane TEIL 2005-2008</TD></TR></TABLE>
<?php
    // Recherche de nouvelle version
    if ($CHECK_VERSION) {
      $current_version = explode('.', $APPLI_VERSION);
      $head_revision = (int) $current_version[0];
      $minor_revision = $current_version[1];
      $errno = 0;
      $errstr = $version_info = '';
      if ($fsock = @fsockopen('www.phenix.gapi.fr', 80, $errno, $errstr)) {
        @fputs($fsock, "GET /download/version.txt HTTP/1.1\r\n");
        @fputs($fsock, "HOST: phenix.gapi.fr\r\n");
        ...
        $latest_version = $latest_head_revision . '.' . $latest_minor_revision;
        if ($latest_head_revision != $head_revision || $minor_revision != $latest_minor_revision) {
          echo "<p style=\"color:red\" align=\"center\">".sprintf(trad("INDEX_NOUVELLE_VERSION"), $latest_version)." <a href=\"http://www.phenix.gapi.fr/\" target=\"pxSite\">http://phenix.gapi.fr/</a></p>";
        }
      }
    }
  }
