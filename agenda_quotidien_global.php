...
<?php
  ...
  $lienAvant = mktime(12,0,0,$moisEnCours,$jourEnCours-1,$anneeEnCours);
  $lienApres = mktime(12,0,0,$moisEnCours,$jourEnCours+1,$anneeEnCours);
  $leJour = mktime(12,0,0,$moisEnCours,$jourEnCours,$anneeEnCours);
...
<?php
    ...
    while (list($sUtilID,$sNomUtil)=each($aUtilPartage)) {
      ...
      if ($autoriseConsult || ($ckAffCache=="O")) {
        ...
        $dateCrt = mktime(12,0,0,$moisEnCours,$jourEnCours,$anneeEnCours);

        //Preparation au decalage horaire
        prepareDecalageH($tzGmt,$tzEte,$tzHiver,$dateCrt);

        if ($AffTZ=="O") {
          $aJournee[1][7] = $aUtilDecalageHoraire[$sUtilID] - $decalageHoraire;
        } else {
          $aJournee[1][7] = 0;
        }
            ...
            if ($zlPrec==4 || ($zlPrec==2 && ($noteCrt!=$enr['age_id'] || ($noteCrt==$enr['age_id'] && (($i-floor($i)==0) || ($i-floor($i)==0.5)))))) {
              ...
              if ($olibelleNote != $libelleNote && $odetailNote != $detailNote) {
                ...
                if (($sUtilID==$idUser) || (!$notePrive && ($AutoAffectT || ($droit_NOTES >= _DROIT_NOTE_STANDARD_SANS_APPR)))) {
                  ...
                  $lien .= ($droitModifNotePerso || $droitModifNoteAffectee) ? "<A href='javascript: affNoteG(".$aJournee[$iCell][3].",".$aUtilDecalageHoraire[$sUtilID].");'><IMG src='image/modif.gif' width='13' height='13' border='0' vspace='1' align='absmiddle' title='".trad('PLGL_MOD_NOTE')."'></A>" : "";
        ...
        if (!$aUtilDoubleJours[$sUtilID]) {
          ...
          for ($i=$aHeureDebutJourneeUtil[$sUtilID];$i<$aHeureFinJourneeUtil[$sUtilID];$i+=1/$zlPrec)
            if ($aJournee[($i-$iHeureMin)*$zlPrec][0]=="0")
              $aJournee[($i-$iHeureMin)*$zlPrec][0]="2";
        }
        $OldClass="";
        for ($i=0;$i<$iDureeJournee;$i++) {
          ...
          if ($aJournee[$i][0]!="2" && $aJournee[$i][0]!="0") {
            ...
            if ($droit_NOTES >= _DROIT_NOTE_STANDARD_SANS_APPR && (($sUtilID==$idUser) || (($aAutoAffect[$sUtilID]==1) || ($droit_NOTES >= _DROIT_NOTE_MODIF_CREATION and $droit_AGENDAS >= _DROIT_AGENDA_TOUS)))) {
              $cursor="pointer ";
              $sOutput .= "    <TD  align=\"center\" class=\"".$Class."\" style=\"cursor:".$cursor."; background:".$bgColor[$i%2]."\" onclick=\"javascript: nvNoteG('".$sd."','".(($i/$zlPrec)+$iHeureMin+$aJournee[1][7])."','','".$sUtilID."','".(($AffTZ=="O") ? $sUtilID : "")."');\" title=\"".sprintf(trad("PLGL_CREER_NOTE"), afficheHeure(($i/$zlPrec)+$iHeureMin+$aJournee[1][7],($i/$zlPrec)+$iHeureMin+$aJournee[1][7],$formatHeure))."\">&nbsp;</TD>\n";
            } else {
              $lienNote="";
              $cursor="default";
