...
// ----------------------------------------------------------------------------
  function formLog() {
    ...
    echo "<html><head><title>".trad("PPX_TITRE")."</title></head>";
    ...
    echo trad("PPX_IDENTIFIANT")."<br><input type=\"text\" name=\"log\" size=\"10\"><br>";
    echo trad("PPX_MOT_DE_PASSE")."<br><input type=\"text\" name=\"pwd\" size=\"10\">";
...
// ----------------------------------------------------------------------------
  if (!isset($sid)) {
    ...
    $DB_CX->DbQuery("SELECT util_id, util_semaine_type FROM ${PREFIX_TABLE}utilisateur WHERE util_login='".$log."' AND util_passwd='".$ztPasswd."'");

    if ($DB_CX->DbNumRows()) {
      // L'utilisateur existe
      $idUser = $DB_CX->DbResult(0,0);
      $SEMAINE_TYPE = $DB_CX->DbResult(0,1);
      // MOD [PPX] Affichage hebdo
      //$SEMAINE_TYPE = ($SEMAINE_TYPE!="1111111" && $SEMAINE_TYPE!="0000000") ? $SEMAINE_TYPE : "1111100";
      $SEMAINE_TYPE = ($SEMAINE_TYPE!="0000000") ? $SEMAINE_TYPE : "1111100";
      // Fin MOD [PPX] Affichage hebdo
...
// ----------------------------------------------------------------------------
  ...
  $localTime = mktime(gmdate("H")+floor($decalageHoraire),gmdate("i")+($decalageHoraire*60)%60,0,gmdate("n"),gmdate("j"),gmdate("Y"));
...
// ----------------------------------------------------------------------------
  elseif ($v==2) {
    ...
    $decalageHoraire = calculDecalageH($tzGmt,$tzEte,$tzHiver,mktime(12,0,0,$moisEnCours,$jourEnCours,$anneeEnCours));
    $enr['ageDate'] = date("d/m/Y",$sd);
    $enr['age_heure_debut'] = (isset($hD) && !empty($hD)) ? $hD+(($id) ? $decalageHoraire : 0) : $DB_CX->DbResult(0,0);
    $enr['age_heure_fin'] = (isset($hF) && !empty($hF)) ? $hF+(($id) ? $decalageHoraire : 0) : $enr['age_heure_debut']+(0.25*$dureeNote);
    // Fin MOD [PPX] Affichage hebdo
    $url = "?sid=".$sid."&sd=".$sd."&v=3&ztAction=INSERT";
    //Recuperation des informations sur la note pour une modification
    if ($id) {
      $DB_CX->DbQuery("SELECT age_id, age_libelle, age_detail, DATE_FORMAT(age_date,'%d/%m/%Y') AS ageDate, age_heure_debut, age_heure_fin, age_prive, age_rappel, age_rappel_coeff, age_aty_id, age_disponibilite, age_lieu, age_couleur, age_cal_id FROM ${PREFIX_TABLE}agenda WHERE age_id=".$id." AND age_util_id=".$idUser." AND age_aty_id!=1");
      if ($enr = $DB_CX->DbNextRow()) {
        ...
        $enr['age_heure_debut'] += $decalageHoraire;
        $enr['age_heure_fin'] += $decalageHoraire;
        // on normalise la date et l'heure de debut
        if ($enr['age_heure_debut'] < 0) {
          $enr['age_heure_debut'] += 24;
          $enr['ageDate'] = date("d/m/Y",mktime(12,0,0,$tabDate[1],$tabDate[0]-1,$tabDate[2]));
        }
...
// ----------------------------------------------------------------------------
  elseif ($v==3) {
    ...
    $sd = mktime(12,0,0,$tabDate[1],$tabDate[0],$tabDate[2]);
    ...
    if (!empty($ztLibelle) && $ztAction=="INSERT") {
      ...
      $sql .= "VALUES (".$idUser.",".$ckTypeNote.",'".$ztDate."',".$zlHeureDebut.",".$zlHeureFin.",'".$ztLibelle."','".$ztDetail."',".$zlR1.",".$zlR2.",".$zlPartage.",".$zlDispo.",'".$ztLieu."','".$zlCouleur."',".$zlContactAssocie.",".$idUser.", '".$dateCreation."',".$idUser.", '".$dateCreation."')";
      $DB_CX->DbQuery($sql);
...
// ----------------------------------------------------------------------------
  elseif ($v==7) {
    ...
    $semaineAvant  = $NOM_PAGE."?sid=".$sid."&v=7&sd=".mktime(12,0,0,$moisEnCours,$premierJourSemaine-7,$anneeEnCours);
    $semaineApres  = $NOM_PAGE."?sid=".$sid."&v=7&sd=".mktime(12,0,0,$moisEnCours,$premierJourSemaine+7,$anneeEnCours);
    echo ("<tr align=\"center\" bgcolor=\"$PPX_COULEUR_FOND_COMMUN\"><td colspan=\"4\">
    ...
    <tr>
      ...
      <table cellspacing=\"0\" cellpadding=\"0\" width=\"100%\" border=\"0\">
        <tr>
          ...
          <td align=\"center\" width=\"90%\" nowrap><font color=\"$PPX_COULEUR_TEXTE_MOIS\"><a href=\"".$NOM_PAGE."?sid=".$sid."&sd=".mktime(12,0,0,$moisEnCours,1,$anneeEnCours)."\">".$tabMois[$moisEnCours*1]." ".$anneeEnCours."</a> [".sprintf(trad("PPX_SEMAINE"), date("W",$debutSemaine))."]</font></td>
...
// ----------------------------------------------------------------------------
  elseif (!$v) {
    ...
    function afficheJour($nbJour, $leJour, $jourSemaine) {
      ...
      echo "<td height=\"19\"".(($leJour==date("Y-m-d",$localTime)) ? " bgcolor=\"$PPX_COULEUR_FOND_JOUR_COURANT\"" : "")."><a href=\"".$NOM_PAGE."?sid=".$sid."&v=1&sd=".mktime(12,0,0,intval($moisEnCours),$nbJour,$anneeEnCours)."\"><font color=\"".$couleurJour."\">".$numJour."</font></a></td>";
    }
    // Menu de navigation entre les differents mois
    $moisAvant  = $NOM_PAGE."?sid=".$sid."&sd=".(($moisEnCours != 1) ? mktime(12,0,0,($moisEnCours-1),1,$anneeEnCours) : mktime(12,0,0,12,1,($anneeEnCours-1)));
    $moisApres  = $NOM_PAGE."?sid=".$sid."&sd=".(($moisEnCours != 12) ? mktime(12,0,0,($moisEnCours+1),1,$anneeEnCours) : mktime(12,0,0,1,1,($anneeEnCours+1)));
    $anneeAvant = $NOM_PAGE."?sid=".$sid."&sd=".mktime(12,0,0,$moisEnCours,1,($anneeEnCours-1));
    $anneeApres = $NOM_PAGE."?sid=".$sid."&sd=".mktime(12,0,0,$moisEnCours,1,($anneeEnCours+1));
    ...
    // MOD [PPX] Affichage hebdo
    //echo ("</tr>\n        <tr align=\"center\"><td width=\"25\" height=\"19\" align=\"right\" bgcolor=\"$PPX_COULEUR_FOND_COLONNE_SEMAINE\" style=\"border-right:solid 1px $PPX_COULEUR_BORDURE_TABLEAU;\">".(date("W",mktime(12,0,0,$moisEnCours, 1, $anneeEnCours))+0)."&nbsp;</td>");
    echo ("</tr>\n        <tr align=\"center\"><td width=\"25\" height=\"19\" align=\"right\" bgcolor=\"$PPX_COULEUR_FOND_COLONNE_SEMAINE\" style=\"border-right:solid 1px $PPX_COULEUR_BORDURE_TABLEAU;\"><a href=\"".$NOM_PAGE."?sid=".$sid."&v=7&sd=".mktime(12,0,0,$moisEnCours, 1, $anneeEnCours)."\">".(date("W",mktime(12,0,0,$moisEnCours, 1, $anneeEnCours))+0)."</a>&nbsp;</td>");
    // Fin MOD [PPX] Affichage hebdo
    ...
    //Preparation au decalage horaire
    prepareDecalageH($tzGmt,$tzEte,$tzHiver,mktime(12,0,0,$moisEnCours,1,$anneeEnCours));
    ...
    for($j=1;!$finDeMois;$j++) {
      if (checkdate($moisEnCours, $nbJour+1, $anneeEnCours)) {
        // MOD [PPX] Affichage hebdo
        //echo "        <tr align=\"center\"><td width=\"25\" height=\"19\" align=\"right\" bgcolor=\"$PPX_COULEUR_FOND_COLONNE_SEMAINE\" style=\"border-right:solid 1px $PPX_COULEUR_BORDURE_TABLEAU;\">".(date("W",mktime(12,0,0,$moisEnCours, $nbJour+1, $anneeEnCours))+0)."&nbsp;</td>";
        echo "        <tr align=\"center\"><td width=\"25\" height=\"19\" align=\"right\" bgcolor=\"$PPX_COULEUR_FOND_COLONNE_SEMAINE\" style=\"border-right:solid 1px $PPX_COULEUR_BORDURE_TABLEAU;\"><a href=\"".$NOM_PAGE."?sid=".$sid."&v=7&sd=".mktime(12,0,0,$moisEnCours, $nbJour+1, $anneeEnCours)."\">".(date("W",mktime(12,0,0,$moisEnCours, $nbJour+1, $anneeEnCours))+0)."</a>&nbsp;</td>";
        // Fin MOD [PPX] Affichage hebdo
        for($i=1;$i<8;$i++) {
    ...
    echo ("        <tr align=\"center\" bgcolor=\"$PPX_COULEUR_LIGNE_AUJOURDHUI\"><td colspan=\"8\" height=\"18\" style=\"border-top:solid 1px $PPX_COULEUR_BORDURE_TABLEAU;\"><a href=\"".$NOM_PAGE."?sid=".$sid."&v=1&sd=".mktime(12,0,0,date("n",$localTime), date("j",$localTime), date("Y",$localTime))."\">".trad("PPX_AUJOURDHUI").": ".date("d/m/Y",$localTime)."</a></td></tr>
      </table></td>
    </tr>
  </table>");
  }
...
// MOD [PPX] Affichage hebdo
//  echo ("&nbsp;</td></tr><tr align=\"center\" bgcolor=\"$PPX_COULEUR_FOND_COPYRIGHT\"><td colspan=\"4\"><a href=\"mailto:phenix-agenda@laposte.net\"><font color=\"$PPX_COULEUR_NOM_APPLI\">Pocket-PheniX ".$APPLI_VERSION."</font></a></td></tr></table>
//</body>
//</html>");
  echo ((($v==7) ? "" : "&nbsp;")."</td></tr><tr align=\"center\" bgcolor=\"$PPX_COULEUR_FOND_COPYRIGHT\"><td colspan=\"4\"><a href=\"mailto:phenix-agenda@laposte.net\"><font color=\"$PPX_COULEUR_NOM_APPLI\">".sprintf(trad("PPX_VERSION_PHENIX"), $APPLI_VERSION)."</font></a></td></tr></table>
</body>
</html>");
// Fin MOD [PPX] Affichage hebdo
?>
