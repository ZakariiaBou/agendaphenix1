...
?>
  ...
  <TR bgcolor="<?php echo $bgColor[1]; ?>">
    ...
    <TD class="tabInput" colspan="2" width="436"><TABLE cellspacing="0" cellpadding="0" width="100%" border="0" align="center">
      ...
      <TR>
        <TD><SELECT name="zlUtilisateur" id="zlUtilisateur" size="8" multiple style="width:200px;border: <?php echo $FormulaireBordureInput; ?>;">
<?php
  ...
  if ($isGlobalQuot || $isDisponibilite) {
    if ($isGlobalQuot && !empty($sChoix)) {
      ...
      while ($enr=$DB_CX->DbNextRow()) {
        ...
        $aUtiltzEte = calculBasculeDST($enr['tzn_date_ete'],gmdate("Y"),$enr['tzn_heure_ete'],$aUtiltzGmt[$enr['util_id']],0);
        $aUtiltzHiver = calculBasculeDST($enr['tzn_date_hiver'],gmdate("Y"),$enr['tzn_heure_hiver'],$aUtiltzGmt[$enr['util_id']],1);
        $aUtilDecalageHoraire[$enr['util_id']] = calculDecalageH($aUtiltzGmt[$enr['util_id']],$aUtiltzEte,$aUtiltzHiver,mktime(12,0,0,$moisEnCours,$jourEnCours,$anneeEnCours));
    ...
    elseif ($isDisponibilite && !empty($sChoix)) {
      // Recuperation de l'heure de debut de journee minimale et de l'heure de fin de journee maximale selectionnees par l'utilisateur
      if ($zlHD!="") {
        $iHeureMin = $zlHD;
      } else {
