  ...
  *  option) any later version.                                              *
  ...
  if (!empty($typeExport)) {
    ...
    if ($typeExport=="rss") {
      ...
      $DB_CX->DbQuery("SELECT util_id, util_format_nom,util_prenom,util_nom,util_langue FROM ${PREFIX_TABLE}utilisateur WHERE util_url_export='".$id."'");
      ...
      $sql  = "SELECT age_id, age_libelle, age_detail, age_prive, age_aty_id, age_date, age_heure_debut, age_heure_fin, age_ape_id, age_periode1, age_periode2, age_periode3, age_periode4, age_couleur, age_date_creation, age_plage, age_plage_duree, util_semaine_type, age_lieu, age_disponibilite, age_rappel, age_rappel_coeff, age_email, age_cal_id,CONCAT(".$FORMAT_NOM_UTIL.") AS nomCreateur ,util_email ";
      $sql .= "FROM ${PREFIX_TABLE}agenda, ${PREFIX_TABLE}agenda_concerne, ${PREFIX_TABLE}utilisateur ";
      $sql .= "WHERE aco_util_id=".$idUser." AND age_id=aco_age_id AND util_id=age_createur_id";
      if ($idAge!="") {
      ...
      if ($idAge!="") {
        $DB_CX->DbQuery($sql);
        $enr = $DB_CX->DbNextRow();
        if (($enr['age_detail']=="") && ($enr['age_lieu']=="")) echo "<i>".$enr['age_libelle']."</i>";
        else echo $enr['age_detail'];
        if ($enr['age_lieu']!="") echo "<br>".trad("NOTEEXP_RSS_EMPLACEMENT")." : ".$enr['age_lieu'];
      } else {
        ...
        $contenu .= "<title>Agenda Phenix</title>";
        ...
        while ($enr = $DB_CX->DbNextRow()) {
          ...
          list($an_debut,$mois_debut,$jour_debut) = explode("-",$enr['age_date']);
          $decalageHoraire = calculDecalageH($tzGmt,$tzEte,$tzHiver,mktime(12,0,0,$mois_debut,$jour_debut,$an_debut));
          ...
          if ($enr['age_heure_debut'] < 0) {
            ...
            $enr['age_date'] = date("Y-m-d",mktime(12,0,0,$mois_debut,$jour_debut-1,$an_debut));
          }
          ...
          // On met le la date de debut+heure au bon format
          list($note_a,$note_m,$note_j) = explode("-",$enr['age_date']);
          // date de debut de la note au format : 2006-12-14 13:53:39
          $date_deb_fmt = $note_a."-".$note_m."-".$note_j." ".$heure_debut.":00";
          ...
          $contenu .= "<pubDate>".str_replace(" "," ",date("r",strtotime($date_deb_fmt)))."</pubDate>";
          $contenu .= "<author>".$enr['nomCreateur']." &lt;".$enr['util_email']."&gt;</author>";
          $contenu .= "</item>";
        }
        $contenu .= "</channel>";
        $contenu .= "</rss>";
    ...
    elseif ($typeExport=="ics") {
      ...
      if ($subTypeExport == "URL") {
        // Si depuis URL PHENIX
        //Date de debut et de fin en fonction des notes de l'utilisateur identifie par l'ID passe en parametre dans l'URL
        $DB_CX->DbQuery("SELECT DATE_FORMAT(MIN(age_date),'%d/%m/%Y'), DATE_FORMAT(MAX(age_date),'%d/%m/%Y'), util_id, util_format_nom, util_langue FROM ${PREFIX_TABLE}agenda, ${PREFIX_TABLE}agenda_concerne, ${PREFIX_TABLE}utilisateur WHERE age_aty_id>1 AND age_mere_id=0 AND age_id=aco_age_id AND aco_util_id=util_id AND util_url_export='".$id."' GROUP BY util_id");
        if ($DB_CX->DbResult(0,0)!=NULL) {
          $ztDateDeb = $DB_CX->DbResult(0,0);
          $ztDateFin = $DB_CX->DbResult(0,1);
        } else {
          $ztDateDeb = gmdate("d/m/").(gmdate("Y")-5);
          $ztDateFin = gmdate("d/m/").(gmdate("Y")+5);
        }
      ...
      // Recuperation des infos de timezone de l'utilisateur
      $DB_CX->DbQuery("SELECT tzn_libelle, tzn_gmt, tzn_date_ete, tzn_heure_ete, tzn_date_hiver, tzn_heure_hiver, tzn_zone FROM ${PREFIX_TABLE}utilisateur, ${PREFIX_TABLE}timezone WHERE util_id=".$idUser." AND tzn_zone=util_timezone");
      $tzLibelle = htmlentities($DB_CX->DbResult(0,0));
      $tzGmt = $DB_CX->DbResult(0,1);
      $tzEte = calculBasculeDST($DB_CX->DbResult(0,2),gmdate("Y"),$DB_CX->DbResult(0,3),$tzGmt,0);
      $tzHiver = calculBasculeDST($DB_CX->DbResult(0,4),gmdate("Y"),$DB_CX->DbResult(0,5),$tzGmt,1);
      $tzn_zone = htmlentities($DB_CX->DbResult(0,6));
      if ($ckExportTz!="1") {
        if ($subTypeExport!="MS") {
          $TZID = ";TZID=/mozilla.org/20070129_1/".$tzn_zone;
        } else {
          $TZID = ";TZID=".$tzn_zone;
        }
      } else {
      ...
      if ($rdChExport!="1") {
        ...
        $decalageHoraire = calculDecalageH($tzGmt,$tzEte,$tzHiver,mktime(12,0,0,$m_deb,$j_deb,$a_deb));
        $date_dem_deb = date("Y-m-d H.i",mktime(0-floor($decalageHoraire),0-($decalageHoraire*60)%60,0,$m_deb,$j_deb,$a_deb));
        $decalageHoraire = calculDecalageH($tzGmt,$tzEte,$tzHiver,mktime(12,0,0,$m_fin,$j_fin,$a_fin));
        $date_dem_fin = date("Y-m-d H.i",mktime(24-floor($decalageHoraire),0-($decalageHoraire*60)%60,0,$m_fin,$j_fin,$a_fin));
      } else {
        // Rajout de l'heure pour les dates de creation
        $date_dem_deb = str_replace(" ", "/", $ztDateDebC);
        $date_dem_fin = str_replace(" ", "/", $ztDateFinC);
        list($j_deb,$m_deb,$a_deb,$h_deb) = explode("/",$date_dem_deb);
        list($j_fin,$m_fin,$a_fin,$h_fin) = explode("/",$date_dem_fin);
        if ($h_deb=="")
          $h_deb="00:00:00";
        if ($h_fin=="")
          $h_fin="00:00:00";
        $date_dem_deb=$a_deb."-".$m_deb."-".$j_deb." ".$h_deb;
        $date_dem_fin=$a_fin."-".$m_fin."-".$j_fin." ".$h_fin;
      }
      ...
      if (!$DB_CX->DbNumRows()) {
        ...
        if ($rdChExport!="1") {
          ...
          $sql .= " AND CONCAT(age_date,' ',LEFT(CONCAT('0',age_heure_debut),5))>='".$date_dem_deb."' AND CONCAT(age_date,' ',LEFT(CONCAT('0',age_heure_debut),5))<='".$date_dem_fin."'".$exclusion;
        } else {
          $sql  = "SELECT age_id, age_libelle, age_detail, age_prive, age_aty_id, age_date, age_heure_debut, age_heure_fin, age_ape_id, age_periode1, age_periode2, age_periode3, age_periode4, age_couleur, age_date_creation, age_date_modif, age_plage, age_plage_duree, util_semaine_type, age_lieu, age_disponibilite, age_rappel, age_rappel_coeff, age_email, age_cal_id ";
          $sql .= "FROM ${PREFIX_TABLE}agenda, ${PREFIX_TABLE}agenda_concerne, ${PREFIX_TABLE}utilisateur ";
          $sql .= "WHERE aco_util_id=".$idUser." AND age_id=aco_age_id AND age_aty_id>1 AND age_mere_id=0 AND util_id=age_createur_id";
        ...
        if ($DB_CX->DbNumRows()) {
          ...
          while ($enr = $DB_CX->DbNextRow()) {
            ...
            if ($ckExportTz!="1") {
              ...
              $decalageHoraire = calculDecalageH($tzGmt,$tzEte,$tzHiver,mktime(12,0,0,$m_deb,$j_deb,$a_deb));
              list($enr['age_heure_debut'],$enr['age_heure_fin'],$enr['dateCreation'],$enr['dateModif']) = decaleNote($tzGmt,$tzEte,$tzHiver,date("Y-m-d",mktime(12,0,0,$m_deb,$j_deb,$a_deb)),$enr['age_date'],$enr['age_heure_debut'],$enr['age_heure_fin'],$enr['age_date_creation'],$enr['age_date_modif']);
            ...
            $data .= "DTSTAMP:".str_replace("-","",$date_create)."T".$heure_create.$min_create."00Z\r\n";
             // Date de modification de la note : 2006-10-17 11:08:02
            ...
            list($a_deb,$m_deb,$j_deb) = explode ("-",$enr['age_date']);
            $date_fin = ($enr['age_heure_debut']>$enr['age_heure_fin']) ? date("Y-m-d",mktime(12,0,0,$m_deb,$j_deb+1,$a_deb)) : $enr['age_date'];
            // On enregistre les heures de debut et de fin de la note
            $data .= "DTSTART".$TZID.":".str_replace("-","",$enr['age_date'])."T".$heure_deb.$min_deb."00Z\r\n";
            $data .= "DTEND".$TZID.":".str_replace("-","",$date_fin)."T".$heure_fin.$min_fin."00Z\r\n";
            ...
            if ($enr['age_rappel']!=0) {
              ...
              $decalageHoraire = calculDecalageH($tzGmt,$tzEte,$tzHiver,mktime(12,0,0,gmdate("m"),gmdate("d"),gmdate("Y")));
              $date_actuelle = date("Ymd Hi",mktime(gmdate(H)-floor($decalageHoraire),gmdate(i)-($decalageHoraire*60)%60,0,gmdate("m"),gmdate("d"),gmdate("Y")));
              $data .= "X-MOZ-LASTACK:".substr($date_actuelle,0,8)."T".substr($date_actuelle,-4)."00Z\r\n";
              $data .= "ACTION:DISPLAY\r\n";
              $data .= "END:VALARM\r\n";
            }
            $data .= "END:VEVENT\r\n";
    ...
    elseif ($typeExport=="vcs") {
      ...
      // Recuperation des infos de timezone de l'utilisateur
      $DB_CX->DbQuery("SELECT tzn_libelle, tzn_gmt, tzn_date_ete, tzn_heure_ete, tzn_date_hiver, tzn_heure_hiver FROM ${PREFIX_TABLE}utilisateur, ${PREFIX_TABLE}timezone WHERE util_id=".$idUser." AND tzn_zone=util_timezone");
      $tzLibelle = htmlentities($DB_CX->DbResult(0,0));
      $tzGmt = $DB_CX->DbResult(0,1);
      $tzEte = calculBasculeDST($DB_CX->DbResult(0,2),gmdate("Y"),$DB_CX->DbResult(0,3),$tzGmt,0);
      $tzHiver = calculBasculeDST($DB_CX->DbResult(0,4),gmdate("Y"),$DB_CX->DbResult(0,5),$tzGmt,1);
    ...
    elseif ($typeExport=="csv") {
      ...
      if ($rdChExport!="1") {
        ...
        $date_dem_deb = preg_replace(" ", "/", $ztDateDebC);
        $date_dem_fin = str_replace(" ", "/", $ztDateFinC);
        list($j_deb,$m_deb,$a_deb,$h_deb) = explode("/",$date_dem_deb);
        list($j_fin,$m_fin,$a_fin,$h_fin) = explode("/",$date_dem_fin);
        $date_dem_deb=$a_deb."-".$m_deb."-".$j_deb." ".$h_deb;
      ...
      if (!$DB_CX->DbNumRows()) {
        ...
        if ($DB_CX->DbNumRows()) {
          ...
          while ($enr = $DB_CX->DbNextRow()) {
            ...
            $Fin_date = ($enr['age_heure_debut']>$enr['age_heure_fin']) ? date("d/m/Y",mktime(12,0,0,$m_deb,$j_deb+1,$a_deb)) : $Debut_date;
  ...
  // Generation du module d'export dans PHENIX
    ...
    $ckSauvSql=0;
    $DB_CX->DbQuery("SELECT aex_html, aex_tz, aex_creation, aex_type, aex_ch_note, aex_note_aff , DATE_FORMAT(aex_date_old,'%d/%m/%Y %H:%i:%s') FROM ${PREFIX_TABLE}agenda_export WHERE aex_util_id=".$idUser);
    if ($DB_CX->DbNumRows()) {
    ...
    $DB_CX->DbQuery("SELECT DATE_FORMAT(MIN(age_date),'%d/%m/%Y'), DATE_FORMAT(MAX(age_date),'%d/%m/%Y') FROM ${PREFIX_TABLE}agenda, ${PREFIX_TABLE}agenda_concerne WHERE age_id=aco_age_id AND aco_util_id=".$idUser." AND age_aty_id>1 AND age_mere_id=0");
    if ($DB_CX->DbResult(0,0)!=NULL) {
      $ztDateDeb = $DB_CX->DbResult(0,0);
      $ztDateFin = $DB_CX->DbResult(0,1);
    } else {
      ...
      $ztDateFinC= date("d/m/Y H:i:s",$localTime);
      if (($ztDateDebC=="00/00/0000 00:00:00") || ($ckSauvSql=="0")) {
        $MessAff=1;
        $ztDateDebC = $ztDateDeb." 00:00:00";
      }
?>
